...
export const Tournaments: React.FC<TournamentsProps> = ({
  ...
  const handleConfirmTableBreak = (breakTable: string) => {
    ...
    if (isFinal) {
      const finalRedraw = calculateFinalTableRedraw(seating, activeTournament, state.members);
      ...
      assignments = calculateBreakAssignments(seating, breakTable, activeTournament);
      updated = executeTableBreak(seating, breakTable, activeTournament);
    }
  ...
  <div style="background-color: #ffffff; padding: 40px; border-bottom-left-radius: 12px; border-bottom-right-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
    ...
    <p style="font-size: 12px; color: #9ca3af; text-align: center; margin: 0;">&copy; 2026 Penny Ante Poker Club. All rights reserved.</p>
      ...
      {subTab === 'checkin' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }} className="animate-slide-up">
          {activeTournament.seatingLocked && (
            <div style={{ backgroundColor: 'rgba(239, 68, 68, 0.1)', border: '1.5px solid #ef4444', borderRadius: '8px', padding: '12px 16px', color: '#ffffff', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
              ⚠️ REGISTRATION LOCKED: Seating assignments have been generated. Late entries must be added using the seating chart late entry button.
            </div>
          )}
          {activeTournament.status === 'draft' ? (
            !isSubAdmin && !activeTournament.seatingLocked && renderFastPlayerLookup("Register a Player", checkinSearchRef)
          ) : (
...
<thead>
                      <tr>
                        ...
                        {activeTournament.status === 'draft' && !activeTournament.seatingLocked && <th style={{ textAlign: 'right' }}>Action</th>}
                      </tr>
                    </thead>
                    <tbody>
                      {items.map(({ entry, member: m }) => (
                        <tr key={entry.memberId}>
                          ...
                          {!isSubAdmin && activeTournament.status === 'draft' && !activeTournament.seatingLocked && (
                            <td style={{ textAlign: 'right' }}>
                              <button
                                onClick={() => unregisterPlayer(activeTournament.id, entry.memberId)}
                                className="btn btn-ghost"
      ...
      {subTab === 'summary' && (() => {
        ...
        return (
          ...
          <EliminationModal
            ...
            onConfirm={(bounties) => {
              ...
              if (activeTournament && eliminatingPlayerId) {
                ...
                const rec = checkTableBalance(seating, simulatedTournament, state.members);
                if (rec) {
          ...
          <ResumeClockModal
            isOpen={!!showResumeClockModal && !!activeTournament}
